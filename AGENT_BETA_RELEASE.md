# ChainWhisper Agent Public Beta

## Release scope

This public beta is prepared from the `agent` branch. It contains two distinct
agent experiences:

- The in-app Agent area at `/otc/agent` keeps App Help, the paid WISP Trade
  Agent, and Agent Setup inside the ChainWhisper interface.
- The external Agent Setup path installs
  `@chainwhisper/agent-tools@0.1.0-beta.0`, which provides a keyless
  ChainWhisper planner and a local credential-holding COTI signer as two paired
  stdio MCP connections.

App Help remains free and local-first. In-app Trade Agent requests remain
separately paid with WISP. External MCP transactions do not pay that request
fee; they pay normal COTI gas and the current ChainWhisper contract fee.
...
- Agent Setup remains free and has no wallet, payment, signature, or WISP side
  effects.
- App CI runs once on Ubuntu with Node.js 20.
- MCP package CI runs on Ubuntu with Node.js 20, 22, and 24, and on Windows with
  Node.js 22.
...
Baseline verification of commit `a1d3951` and its MCP package on July 28, 2026:

- Lint passed with no errors.
- The app unit suite passed: 81 files and 715 tests.
- The focused Agent suite passed: 14 files and 94 tests.
- The production TypeScript and Vite build passed.
- The MCP TypeScript build passed.
- The MCP suite passed: 18 files and 172 tests.
- The stdio smoke verified the planner and the unconfigured signer's safe
  read-only status.
- The browser suite passed: 85 tests with no skips.
- The Trade Agent Edge Function passed its Deno check and 13 tests.
- Live read-only status and runtime audit passed against COTI Mainnet.
- No transaction was signed or broadcast and no private message was sent during
  the release verification.

The strengthened external npm-install and command-shim gate, expanded
cross-platform MCP matrix, documentation, security scan, and publishing
workflow are post-baseline release-candidate changes. GitHub CI must attest the
final commit before publication.

Integrated public-beta hardening verified locally on July 29, 2026:

- Lint, the 81-file / 715-test app suite, and the production build passed.
- The MCP TypeScript build and 24-file suite passed: 209 tests passed and three
  POSIX permission checks were skipped on Windows.
- The built stdio smoke passed without credentials, network writes, signing, or
  private-message sends.
- The read-only COTI Mainnet audit passed at block `0x820702`: every committed
...
- Require the final app job and every MCP matrix job to pass on GitHub.
...
No migration, Edge Function deployment, GitHub Pages deployment, merge to
`main`, transaction broadcast, or private-message write is implied by preparing
the npm beta.
